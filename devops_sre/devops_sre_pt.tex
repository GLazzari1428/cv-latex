...
\begin{itemize}
	\item Implantei stack completa de monitoramento: Prometheus para coleta de m\'{e}tricas, Grafana para visualiza\c{c}\~{a}o e alertas, Scrutiny para an\'{a}lise preditiva de falhas de disco via dados S.M.A.R.T., e Glance para dashboards centralizados.
	\item Projetei arquitetura de acesso em m\'{u}ltiplas camadas: VPN WireGuard com proxy reverso Nginx para servi\c{c}os internos (\texttt{glza.xyz}), Cloudflare Zero Trust com TOTP para endpoints p\'{u}blicos (\texttt{glza.cloud}).
...
\begin{description}
	...
	\item[Monitoramento \& Observabilidade] Prometheus, Grafana, Scrutiny (S.M.A.R.T.), Glance, an\'{a}lise de logs
