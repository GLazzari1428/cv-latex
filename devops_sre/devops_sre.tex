...
\begin{itemize}
	\item Deployed comprehensive monitoring stack: Prometheus for metrics collection, Grafana for visualization and alerting, Scrutiny for predictive disk failure analysis via S.M.A.R.T. data, and Glance for centralized dashboarding.
	\item Engineered multi-layer access architecture: WireGuard VPN with Nginx reverse proxy for internal services (\texttt{glza.xyz}), Cloudflare Zero Trust with TOTP for public endpoints (\texttt{glza.cloud}).
...
\begin{description}
	...
	\item[Monitoring \& Observability] Prometheus, Grafana, Scrutiny (S.M.A.R.T.), Glance, log analysis
